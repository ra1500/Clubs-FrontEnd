...
class MessageBoardFriend extends React.Component {
  constructor(props) {
    super(props);
  ...
  postMessage() {
    ...
    let data = {message : this.state.clubMessage, receiverType: 4 };
    axios.post("http://localhost:8080/api/m/e?fId=" + this.props.friendshipsEntityId, data,
    ...
    getMessagesList() {
        ...
        const hash = btoa(token);
        const Basic = 'Basic ' + hash;
        axios.get("http://localhost:8080/api/m/d?fId=" + this.props.friendshipsEntityId + "&pN=" + this.state.scrollPageNumber,
        {headers : { 'Authorization' : Basic }})
        .then((response) => {
  ...
  render() {
    ...
    <div id="meSettingsDiv">
        ...
        <p class="noLineSpaceP"> Network Contact to Network Contact Message Board </p>
...
export default MessageBoardFriend;